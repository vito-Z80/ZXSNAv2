...
class TextureLayer(var width: Int, var height: Int) : TextureLayerUtils() {
    ...
    var imageRectangles =
        0             // кол-во прямоугольников в выделенной области в которых нарисован ъотя бы 1 пиксель
    ...
    /**
     * Получить из выделения все внутренние квадраты размером size
     */
    fun getSlicesBounds(rows: Int, columns: Int, size: Int) = Array<RectInt>(rows * columns) {
        val x = it % columns * size
        val y = it / columns * size
        RectInt(x, y, size, size)
    }
